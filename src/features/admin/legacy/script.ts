...
function runAdminAction(record: LegacyRecord, action: string, reason: string, reasonCode?: AdminQuestReasonCode): Promise<void> {
  ...
  if (action === "Restore quest") {
    return adminCommands.restoreQuest(record.id, {
      ...expectedVersion,
      idempotencyKey,
      reason,
