...
function renderQuestPage() {
  ...
  main.innerHTML = `<div class="record-breadcrumb"><a href="/">Quests</a><span>›</span><span>${questRecord.id}</span></div><div class="full-record-head"><div><div class="record-id">${questRecord.id}</div><h1>${questRecord.title}</h1><p>${questRecord.teamQuest ? "Team quest · " : ""}${questRecord.other} · created by ${questRecord.person}</p></div><div class="full-record-actions"><a class="btn" href="/">Back to list</a><button class="btn">More actions</button><button class="btn primary" data-page-action="Approve quest">Approve quest</button></div></div><div class="record-status-bar"><div><span>Status</span>${badge(questRecord.status, questRecord.tone)}</div><div><span>Funded wage</span><strong>฿${fmt(questRecord.amount)}</strong></div><div><span>Participant mode</span><strong>${questRecord.teamQuest ? "Team" : "Single"}</strong></div><div><span>Candidate mode</span><strong>${questRecord.candidateMode || (questRecord.status === "Open" ? "FCFS" : "CANDIDATE")}</strong></div><div><span>Proof</span><strong>${detail.proof.length ? `${detail.proof.length} ${detail.proof.length === 1 ? "file" : "files"}` : "No submission"}</strong></div></div><div class="full-record-grid"><div class="record-primary"><section class="record-panel"><div class="record-panel-head"><h2>Quest brief</h2></div><p class="record-description">${detail.description}</p><div class="requirement-box"><strong>Completion requirements</strong><ul><li>Submit work before the recorded deadline</li><li>Attach verifiable proof files</li><li>Keep communication and payment inside KuQuest</li></ul></div></section>${giverAttachmentsPanel(detail.giverAttachments)}<section class="record-panel"><div class="record-panel-head"><h2>${participantSectionTitle(questRecord, participants, started)}</h2><span class="section-count">${participants.length}</span></div>${relatedRows(participants)}</section>${proofSubmissionsPanel(detail.proof, questRecord)}${renderQuestEditHistory(questRecord)}<section class="record-panel"><div class="record-panel-head"><h2>Complete activity</h2><button class="link">Export log</button></div>${timeline(detail.activity)}</section></div><aside class="record-side"><section class="record-panel"><h2>Giver</h2>${giverProfile(detail)}<button class="btn full-width">Open user profile</button></section><section class="record-panel"><h2>Schedule and location</h2><div class="side-facts"><div><span>Starts</span><strong>${detail.schedule[0]}</strong></div><div><span>Due</span><strong>${detail.schedule[1]}</strong></div><div><span>Application window</span><strong>${detail.schedule[2]}</strong></div><div><span>Location</span><strong>${detail.location[0]}</strong><small>${detail.location[1]}</small></div></div></section><section class="record-panel"><h2>Financial record</h2><div class="financial-line"><span>Funded by giver</span><strong>฿${fmt(questRecord.amount)}</strong></div><div class="financial-line"><span>Platform fee</span><strong>฿${fmt(Math.round(questRecord.amount * 0.05))}</strong></div><div class="financial-line total"><span>${questRecord.teamQuest ? "Team receives (total)" : "Hunter receives"}</span><strong>฿${fmt(Math.round(questRecord.amount * 0.95))}</strong></div><p class="audit-note">Funds remain held until submitted proof is accepted or a dispute is resolved.</p></section><section class="record-panel dispute-summary ${relatedDispute ? "has-dispute" : ""}"><div class="record-panel-head"><h2>Dispute and risk</h2>${relatedDispute ? badge(relatedDispute.status, relatedDispute.tone) : badge("Clear", "success")}</div>${relatedDispute ? `<p><strong>${relatedDispute.id}</strong> · ${relatedDispute.detail}</p><div class="dispute-money"><span>Amount held</span><strong>฿${fmt(relatedDispute.amount)}</strong></div><a class="btn primary full-width" href="/disputes/${encodeURIComponent(relatedDispute.id)}">Open full dispute</a>` : '<div class="no-dispute">No dispute or active moderation hold is connected to this quest.</div>'}</section></aside></div>`;
  main
  ...
  if (questRecord.status === "Change pending") {
    ...
    if (approveButton) {
      approveButton.removeAttribute("data-page-action");
      approveButton.textContent = "Awaiting both parties";
  ...
  if (isBlocked) {
    const approveButton = main.querySelector(
      ...
      disputeUrl = `/disputes/${encodeURIComponent(relatedDispute?.id || "")}`,
    ...
    blocker.innerHTML = `<span>${ico("scale")}</span><div class="decision-block-content"><strong>Quest progression is blocked by ${relatedDispute?.id || "an active dispute"}</strong><dl class="dispute-context"><div><dt>Dispute type</dt><dd>${relatedDispute?.disputeType || disputeCaseFor(relatedDispute || {})?.category || "Other"}</dd></div><div><dt>Description</dt><dd>${relatedDispute?.detail || "A dispute is active for this quest. Open the case to review its description and evidence."}</dd></div></dl></div><a class="btn" href="${disputeUrl}">Review case</a>`;
    ...
    if (disputeSummary && relatedDispute) {
      ...
      context.innerHTML = `<div><dt>Type</dt><dd>${relatedDispute.disputeType || "Other"}</dd></div><div><dt>Description</dt><dd>${relatedDispute.detail}</dd></div>`;
