...
export function MemberDetail({ memberId, initialModel = null, initialTab = "overview", drawer = false }: MemberDetailProps) {
  ...
  if (!model) {
    return <main className={drawer ? "drawer-body" : "admin-feedback"}><Card as="section" className="panel"><h1>{translateText("Member not found")}</h1><p>{translateText(loadError || "No Member record matches this identifier.")}</p>{!drawer && <Link className="btn primary" href={memberRoutes.list()}>{translateText("Return to Members")}</Link>}</Card></main>;
  }
