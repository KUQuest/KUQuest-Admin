...
import { disputeAmountLabel, mockDataNotice } from "./dispute-detail";
...
function evidenceRows(
  record: { id: string; apiBacked?: boolean; evidence: string[]; evidenceRefs?: string[] },
...
function renderDisputePage(): void {
  ...
  ${mockDataNotice(disputeRecord, escapeActivityText)}
  <div class="record-status-bar"><div><span>Status</span>${badge(caseStatus, disputeRecord.tone)}</div><div><span>Category</span><strong>${escapeActivityText(disputeTypeLabel(disputeRecord))}</strong></div><div><span>Dispute date</span><strong>${escapeActivityText(disputeRecord.disputeDate)}</strong></div><div><span>Amount at risk</span><strong>${escapeActivityText(disputeAmountLabel(disputeRecord, fmt))}</strong></div><div><span>Evidence</span><strong>${disputeRecord.evidence.length} records</strong></div></div>
  <div class="full-record-grid"><div class="record-primary">
    ...
    <section class="record-panel"><h2>${isClosed ? "Recorded outcome" : "Resolution decision"}</h2>${isClosed ? `<p class="audit-note">${escapeActivityText(disputeRecord.resolution || "The final allocation was recorded and the case is now read-only.")}</p><div class="overview-group"><span>Reason for this decision</span><p>${escapeActivityText(disputeRecord.decisionReason || "No written reason was recorded for this legacy demo case.")}</p></div>${disputeRecord.penaltyOutcome ? `<div class="overview-group"><span>Additional enforcement</span><p>${escapeActivityText(disputeRecord.penaltyOutcome)}</p></div>` : ""}` : `<div class="allocation dispute-page-allocation"><button data-allocation="hirer"><span>Dismiss Case</span><strong>No money movement</strong></button><button data-allocation="worker"><span>Release to Worker</span><strong>${escapeActivityText(disputeAmountLabel(disputeRecord, fmt))}</strong></button></div><p class="audit-note">Choose the outcome before resolving. A resolved case requires a Worker and a positive Satang amount.</p>`}</section>
  </aside></div>`;
