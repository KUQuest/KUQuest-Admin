...
function closedDecisionSummary(record) {
  const reason = String(
    ...
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#039;");
  return `<section class="section"><h3>Recorded outcome</h3><p class="audit-note">${record.resolution || "The final allocation was recorded and the case is now read-only."}</p></section><section class="section"><h3>Reason for this decision</h3><p>${reason}</p></section>${record.penaltyOutcome ? `<section class="section"><h3>Additional enforcement</h3><p>${record.penaltyOutcome}</p></section>` : ""}`;
}

function partyChats(caseData) {
  return `<section class="section party-chats"><h3>Private case messages</h3><p class="chat-intro">Messages are separate for each party and become part of the case audit trail.</p><div class="chat-launches"><button class="party-chat-button" data-chat-role="giver"><span class="avatar">GV</span><span><strong>Chat with giver</strong><small>${caseData.openedBy}</small></span><span>Open</span></button><button class="party-chat-button" data-chat-role="hunter"><span class="avatar">HN</span><span><strong>Chat with hunter</strong><small>${caseData.respondent}</small></span><span>Open</span></button></div></section>`;
}
...
<div class="drawer-body dispute-record ${isClosed ? "closed-case" : "active-case"}">
 ...
 <section class="section"><div class="section-title"><h3>Related quest</h3>${questIndex >= 0 ? `<a class="link open-related-quest" href="/quests/${caseData.questId}">Open full quest</a>` : ""}</div><a class="quest-reference" href="/quests/${caseData.questId}"><span class="file-icon">${ico("quest")}</span><span><strong>${caseData.questId} · ${record.title}</strong><small>View conditions, assignment, proof, and edit history</small></span><span>›</span></a></section>
 ${partyChats(caseData)}
