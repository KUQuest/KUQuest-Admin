...
export function AdminDrawer({
  ...
  return <>
    ...
    <dialog ref={drawerRef} data-slot="admin-drawer" className={cn("drawer open fixed inset-y-0 right-0 z-[45] m-0 flex h-dvh w-full max-w-[640px] flex-col overflow-hidden border-0 bg-admin-surface p-0 shadow-admin", className)} aria-modal="true" aria-labelledby={titleId} tabIndex={-1} open>
      <CardHeader flush data-slot="drawer-header" className="drawer-top sticky top-0 z-[3] flex min-h-[68px] shrink-0 items-center justify-between gap-3 border-b border-admin-border bg-admin-surface/95 px-5 py-3 backdrop-blur-md"><div className="min-w-0"><strong id={titleId} className="block text-[23px] font-semibold leading-[1.3] text-admin-text">{title}</strong><small className="mt-0.5 block text-[15px] leading-[1.4] text-admin-muted">{subtitle}</small></div><Button variant="ghost" size="icon" className="icon shrink-0" type="button" aria-label={closeButtonAriaLabel ?? ariaLabel} onClick={onClose}><span className="close-lines" /></Button></CardHeader>
      <CardContent flush data-slot="drawer-content" className="drawer-body admin-drawer-content min-h-0 flex-1 overflow-y-auto overscroll-contain [scrollbar-gutter:stable] px-6 py-5 pb-7 max-[720px]:px-4 max-[720px]:py-4 max-[720px]:pb-6">{children}</CardContent>
      {actions ? <CardFooter flush data-slot="drawer-footer" className="admin-drawer-footer sticky bottom-0 z-[3] flex shrink-0 flex-col gap-2 border-t border-admin-border bg-admin-surface/95 px-5 py-3 shadow-[0_-6px_18px_rgba(0,0,0,0.09)] backdrop-blur-md [&>[data-slot=button]]:min-h-11 [&>[data-slot=button]]:flex-1 [&>[data-slot=button]]:text-center md:flex-row">{actions}</CardFooter> : null}
    </dialog>
  </>;
}
