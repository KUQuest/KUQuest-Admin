...
export function ActivityLogBoard({ initialData, initialError }: ActivityLogBoardProps) {
  ...
  if (!apiEnabled && !mockEnabled) {
    return (
      <main id="activity-main" className="admin-route-page activity-log-board" tabIndex={-1}>
        ...
        <Card as="section" className="overflow-hidden activity-log-panel" aria-labelledby="activity-unavailable-title">
  ...
  return (
    <main id="activity-main" className="admin-route-page activity-log-board" tabIndex={-1}>
      ...
      <Card as="section" className="overflow-hidden activity-log-panel" aria-labelledby="activity-log-heading">
        ...
        <form className="activity-log-filters" onSubmit={applyFilters}>
          ...
          <div className="activity-filter-actions"><Button variant="primary" type="submit" disabled={loading}>{translateText("Apply filters")}</Button><Button variant="outline" type="button" onClick={clearFilters} disabled={loading}>{translateText("Clear filters")}</Button></div>
        </form>
        <div className="activity-log-toolbar flex min-h-[54px] flex-wrap items-end gap-2 border-b border-admin-border px-3 py-2"><label className="flex min-w-0 max-w-[420px] flex-1 flex-col gap-1 text-sm text-admin-text" htmlFor="activity-search">{translateText("Search loaded activity")}<Input className="h-9 min-h-9 px-3 py-1.5 text-sm" id="activity-search" type="search" value={search} onChange={(event) => { setSearch(event.target.value); setPageNumber(1); }} placeholder={translateText("Search loaded activity")}/></label><PageSizeControls value={pageSize} translateText={translateText} onChange={(size) => { setPageSize(size); setPageNumber(1); }} /><span className="count text-sm text-admin-muted" aria-live="polite">{filteredEntries.length ? pageSize === "all" ? `${translateText("Showing all")} ${filteredEntries.length} ${translateText(filteredEntries.length === 1 ? "result" : "results")}` : `${translateText("Showing")} ${pageStart}–${pageEnd} ${translateText("of")} ${filteredEntries.length} ${translateText(filteredEntries.length === 1 ? "result" : "results")}` : translateText("Showing 0 of 0 results")}</span></div>
        ...
        {loadError ? <div className="activity-log-error" role="alert"><strong>{translateText("Activity log is not available")}</strong><p>{translateText(loadError)}</p><Button variant="outline" type="button" onClick={retry}>{translateText("Try again")}</Button></div> : null}
        ...
        {paginationError ? <div className="activity-log-error" role="alert"><p>{translateText(paginationError)}</p><Button variant="outline" type="button" onClick={loadMore}>{translateText("Try again")}</Button></div> : null}
        {page?.nextCursor ? <div className="activity-log-pagination"><Button variant="outline" type="button" onClick={loadMore} disabled={loading}>{translateText(loading ? "Loading more" : "Load more")}</Button></div> : null}
      </Card>
