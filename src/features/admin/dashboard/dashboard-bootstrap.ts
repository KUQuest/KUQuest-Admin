...
export function loadDashboardData(storage: BrowserStorage): PersistedAdminData {
  ...
  if (stored) {
    const migrated = migrateDisputeSeed(storage, migrateConductReportSeed(storage, stored));
    // Existing v2/v3 sessions and incomplete v4 sessions receive any missing
    // fixture records without replacing Admin decisions already stored.
    return stored.version === previousCanonicalDashboardSeedVersion
      || stored.version === previousDashboardSeedVersion
      || stored.version === dashboardSeedVersion
      ? migrateExpandedMockSeed(storage, migrated)
      : migrated;
  }
